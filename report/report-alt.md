...
With degree-3 features, a corrected filter threshold, and the combined uncertainty-penalty-plus-rollback safeguard in place, the Dyna loop converged in **six iterations** using **39,616 real-environment steps** — 10.1× fewer than the 400,000-step full-order PPO baseline (Stable-Baselines3 2.8.0, [64,64] MLP, 9,731 parameters; hyperparameters follow Zolman et al.'s defaults — characterizing the minimum viable architecture and step count, and thus the tightest fair comparison, remains future work).
...
**What this says about SINDy-RL's scope.** The method delivers on data efficiency: 10.1× fewer real steps than full-order PPO is a genuine advantage, and the best checkpoint achieves 90% success against the baseline's 100%. It delivers on the reduced-order policy goal (Goal 2): 160 terms is 61× smaller than the baseline NN, evaluates as a single dot product, and is compatible with embedded deployment. It does not deliver on interpretability (Goal 3): 160/165 terms is not sparse enough to be readable, and a 160-term polynomial is not auditable in the way a five-term equation would be. What it cannot deliver — on this system, at this data scale — is full sparsity in the sense of a five-term physically interpretable law. The IDP dynamics are not sparse in a degree-3 polynomial basis: 690 of 720 possible surrogate coefficients are nonzero, and the feature matrix is ill-conditioned, so neither more data nor aggressive thresholding can extract a stable sparse decomposition. These are properties of the representation, not of the algorithm.
...
**Goal 2 (reduced-order policy) is achieved.** The distilled polynomial is 61× smaller than the baseline network (160 terms vs. 9,731 parameters), fully preserves 90% closed-loop success, and is compatible with embedded deployment. **Goal 3 (interpretability) is not achieved.** A 160-term polynomial is compact and deployable but is not auditable in the way a five-term equation would be — STLSQ retains 160 of 165 terms, and a threshold ablation across a 500× range of $\lambda$ values confirms this cannot be tuned away (90--95% success, 121--165 terms, never approaching a readable form).
